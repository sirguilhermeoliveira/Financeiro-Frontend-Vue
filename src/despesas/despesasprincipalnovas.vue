          ...
          <form method="POST" id="fdespesa">
            ...
            <fieldset id="usuario">
              <p id="despesa">
                ...
                >
                  <option value="">
                    Tipo de Despesa - Ex.: Aluguel, Energia...
                  </option>
                  <option value="Alimentação">Alimentação</option>
                  <option value="Aplicação CDI">Aplicação CDI</option>
                  <option value="Compras Revenda">Compras Revenda</option>
                  ...
                  <option value="Pagamento Emprestimo">
                    Pagamento Emprestimo
                  </option>
                  <option value="Salarios">Salarios</option>
                  <option value="Saque de Caixa">Saque de Caixa</option>
                  <option value="Taxa Geral">Taxa Geral</option>
                  <option value="Telefonia">Telefonia</option>
                  <option
                    v-for="select in selects"
                    v-bind:key="select.id"
                    :value="select.novadespesa"
                  >
                    {{ select.novadespesa }}
                  </option>
                  <option value="New">+ Novo Tipo Despesa...</option>
                </select>
              </p>
              ...
              <form>
                <div class="modal2borda">
                  ...
                  >
                    ...
                    <option
                      v-for="select_dois in selects_dois"
                      v-bind:key="select_dois.id"
                      :value="select_dois.novoplanocontas"
                    >
                      {{ select_dois.novoplanocontas }}
                    </option>
              ...
              <form>
                <div class="modal2borda">
                  <input
                    ...
                    id="modal2item1"
                    placeholder="Descrição"
                    v-model="select_dois.novoplanocontas"
                  />
                  <select id="modal2item2">
                    <option value="">Plano Contas</option>
                    <option value="Custos Fixos">3 - Custos Fixos</option>
                    <option value="Custos Variáveis">
                      4 - Custos Variáveis
                    </option>
                    <option value="Despesas Fixas">5 - Despesas Fixas</option>
...
export default {
  ...
  methods: {
    ...
    async getSelectDois() {
      const response = await axios
        .get("api/selects_dois")
        .then((response) => {
          this.selects_dois = response.data.data;
        })
        .catch(function (error) {
          console.warn(error);
        });
    ...
    async addNovoPlanoContas() {
      let uri = "api/selects_dois";
      const response = await axios
        .post(uri, this.select_dois)
        .then((response) => {
          this.selects_dois.push({
            select: this.select_dois.novoplanocontas,
          });
          this.getSelectDois();
        });
    },
    async addNovaDespesa() {
      let uri = "api/selects";
      const response = await axios.post(uri, this.select).then((response) => {
        this.selects.push({
          select: this.select.novadespesa,
        });
        this.getSelect();
      });
    },
